...
class AudioController extends Controller
{
    ...
    // Category function handling category/audio filter for audios

    public function category(Category $category)
    {
        $type = request()->segment(2); // ethir category or person

        $audios = Audio::with(['writer', 'category', 'created_by', 'media']);

            $audios = $audios->where('category_id', $category->id)->get;
