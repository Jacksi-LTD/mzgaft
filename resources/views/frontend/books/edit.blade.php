    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.books.update", [$book->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="title">{{ trans('cruds.book.fields.title') }}</label>
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $book->title) }}" required>
                            @if($errors->has('title'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="writer_id" id="writer_id">
                                @foreach($writers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('writer_id') ? old('writer_id') : $book->writer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="approved" id="approved" value="1" {{ $book->approved || old('approved', 0) === 1 ? 'checked' : '' }}>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('frontend.books.storeCKEditorImages') }}', true);
