...
@can('audio_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.audios.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.audio.title_singular') }}
            </a>
            <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                {{ trans('global.app_csvImport') }}
            </button>
            @include('csvImport.modal', ['model' => 'Audio', 'route' => 'admin.audios.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Audio">
            <thead>
                <tr>
                    <th width="10">
...
@can('audio_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.audios.massDestroy') }}",
...
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-Audio').DataTable(dtOverrideGlobals);
