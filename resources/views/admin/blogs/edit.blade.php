    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.blogs.update", [$blog->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('writing_date') ? 'is-invalid' : '' }}" type="text" name="writing_date" id="writing_date" value="{{ old('writing_date', $blog->writing_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('writer') ? 'is-invalid' : '' }}" name="writer_id" id="writer_id">
                    @foreach($writers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('writer_id') ? old('writer_id') : $blog->writer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id">
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $blog->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('approved') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="approved" id="approved" value="1" {{ $blog->approved || old('approved', 0) === 1 ? 'checked' : '' }}>
