    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.surahs.update", [$surah->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $surah->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number') ? 'is-invalid' : '' }}" type="number" name="number" id="number" value="{{ old('number', $surah->number) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Surah::DOWNLOAD_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('download') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="download_{{ $key }}" name="download" value="{{ $key }}" {{ old('download', $surah->download) === (string) $key ? 'checked' : '' }}>
