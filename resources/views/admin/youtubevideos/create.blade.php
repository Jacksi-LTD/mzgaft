...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('app.youtubevideos') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.youtubevideos.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    <label class="required" for="title">{{ trans('app.url') }}</label>
                    <input class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" type="url" name="url"
                           id="title" value="{{ old('url', '') }}" required>
                    @if ($errors->has('url'))
                ...
                <div class="form-group">
                    ...
                    <select required class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}"
...
@section('scripts')
                            ...
                            return loader.file
                                .then(function(file) {
                                    return new Promise(function(resolve, reject) {
                                        ...
                                        xhr.open('POST',
                                            '{{ route('admin.youtubevideos.storeCKEditorImages') }}',
                                            true);
    ...
    <script>
        Dropzone.options.imageDropzone = {
            url: '{{ route('admin.youtubevideos.storeMedia') }}',
